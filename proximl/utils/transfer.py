...
from proximl.exceptions import ConnectionError as ProxiMLConnectionError
...
async def ping_endpoint(
    ...
    Raises:
        ProxiMLConnectionError: If ping never returns 200 after max retries
...
async def upload(endpoint, auth_token, path, show_progress=True):
    ...
    Raises:
        ...
        ProxiMLConnectionError: If upload fails or endpoint ping fails
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        async def _finalize():
            async with session.post(
                ...
                if response.status != 200:
                    text = await response.text()
                    raise ProxiMLConnectionError(f"Finalize failed: {text}")
                return await response.json()
...
async def download(
    ...
    Raises:
        ProxiMLConnectionError: If download fails or endpoint ping fails
        ProxiMLException: For other errors
    """
    # Normalize endpoint URL to ensure it has a protocol
    endpoint = normalize_endpoint(endpoint)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        except (ProxiMLConnectionError, ClientResponseError) as e:
            # If /info endpoint is not available (404) or other error,
            # default to TAR stream mode and continue
            if isinstance(e, ProxiMLConnectionError) and "404" in str(e):
                logging.debug(
                ...
                # For other errors, convert ClientResponseError to ProxiMLConnectionError
                ...
                # For ProxiMLConnectionError, re-raise as-is
                raise
