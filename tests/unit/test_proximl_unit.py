...
_PROXIML_QUERY_TEST_ENV: Dict[str, str] = {
...
async def test_proximl_query_success(mock_open, mock_requests_get, mock_boto3_client):
    ...
    with patch.dict(os.environ, _PROXIML_QUERY_TEST_ENV):
        # Mock the auth config request
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "region": "us-east-1",
...
async def test_proximl_query_auth_error(mock_open, mock_requests_get, mock_boto3_client):
    ...
    with patch.dict(os.environ, _PROXIML_QUERY_TEST_ENV):
        ...
        proximl.auth.get_tokens = MagicMock(
            side_effect=ProxiMLException("Auth failed")
        )
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_generic_auth_error(
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_with_headers(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _PROXIML_QUERY_TEST_ENV):
        ...
        with patch(
            "proximl.proximl.aiohttp.ClientSession",
            return_value=mock_session_ctx,
        ):
            result = await proximl._query(
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_params_validation(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_boolean_params(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_project_uuid_injection(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_502_retry(
    mock_open, mock_requests_get, mock_boto3_client
):
    ...
    with patch.dict(os.environ, _PROXIML_QUERY_TEST_ENV):
        ...
        with patch(
            ...
            with patch(
                "proximl.proximl.asyncio.sleep", new_callable=AsyncMock
            ):
                result = await proximl._query("/test", "GET")
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_json_error_response(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_non_json_error_response(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_client_response_error(
    mock_open, mock_requests_get, mock_boto3_client
):
...
@patch("builtins.open", side_effect=FileNotFoundError)
@mark.asyncio
async def test_proximl_query_max_retries_exceeded(
    mock_open, mock_requests_get, mock_boto3_client
):
